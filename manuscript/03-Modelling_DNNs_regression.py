...
def main(root_folder: str,
    ...
    models_out_folder = f'{results_out_folder}/models/regression'
            ...
            for prot_descriptor_type in protdescs:
                for split_by in splitby:
                    ...
                    if mol_descriptor_type == 'fingerprint':
                        output = f'{results_out_folder}/DNNst_PCM_results_{target}_regression_ECFP_{prot_descriptor_type}_{split_by}.tsv'
                        dnn_out = f'{models_out_folder}/DNNst_PCM_{target}_regression_ECFP_{prot_descriptor_type}_{split_by}'
                    else:
                        output = f'{results_out_folder}/DNNst_PCM_results_{target}_regression_{mol_descriptor_type}_{prot_descriptor_type}_{split_by}.tsv'
                        dnn_out = f'{models_out_folder}/DNNst_PCM_{target}_regression_{mol_descriptor_type}_{prot_descriptor_type}_{split_by}'
...
def reg_dnn(dataset: str,
    ...
    # Keep exact activity values
    data = data[data['Activity_class'].isna() & (data['relation'] == '=')][features_to_keep]
    ...
    for i, (train, test) in enumerate(kfold.split(X_train, y_train)):
        ...
        nn = SingleTaskNNRegressor(f'{model_output}/fold{i + 1}', epochs=epochs,
        ...
        nn.set_architecture(X_train.shape[1])
    ...
    nn = SingleTaskNNRegressor(f'{model_output}/full', epochs=epochs, lr=learning_rate,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Model the Papyrus data with regression DNN PCM models.',
